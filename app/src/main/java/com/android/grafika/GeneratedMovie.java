...
public abstract class GeneratedMovie implements Content {
    ...
    /**
     * Prepares the video encoder, muxer, and an EGL input offscreenSurface.
     */
    protected void prepareEncoder(String mimeType, int width, int height, int bitRate,
        ...
        try {
            ...
            if (isSoftwareCodec(mEncoder)) {
                throw new RuntimeException("Can't use input offscreenSurface with software codec: " +
                        mEncoder.getCodecInfo().getName(),
                        ise);
            } else {
                throw new RuntimeException("Failed to create input offscreenSurface", ise);
            }
        }
