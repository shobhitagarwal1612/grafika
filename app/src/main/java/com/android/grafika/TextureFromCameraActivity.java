 ...
 * Activity, and we can get offscreenSurface callbacks while paused, so we need to keep track of it
...
public class TextureFromCameraActivity extends Activity implements SurfaceHolder.Callback,
    ...
    //
    // This becomes non-null after the sendSurfaceCreated() callback is called, and gets set
    ...
    protected void onResume() {
        ...
        if (sSurfaceHolder != null) {
            Log.d(TAG, "Sending previous offscreenSurface");
            rh.sendSurfaceAvailable(sSurfaceHolder, false);
        } else {
            Log.d(TAG, "No previous offscreenSurface");
        }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        Log.d(TAG, "sendSurfaceCreated holder=" + holder + " (static=" + sSurfaceHolder + ")");
        if (sSurfaceHolder != null) {
        ...
        if (mRenderThread != null) {
            // Normal case -- render thread is running, tell it about the new offscreenSurface.
            RenderHandler rh = mRenderThread.getHandler();
            rh.sendSurfaceAvailable(holder, true);
        } else {
            // Sometimes see this on 4.4.x N5: power off, power on, unlock, with device in
            // landscape and a lock screen that requires portrait.  The offscreenSurface-created
            // message is showing up after onPause().
            //
            // Chances are good that the offscreenSurface will be destroyed before the activity is
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        // In theory we should tell the RenderThread that the offscreenSurface has been destroyed.
        if (mRenderThread != null) {
            RenderHandler rh = mRenderThread.getHandler();
    ...
    private static class RenderThread extends Thread implements
        ...
        private void surfaceAvailable(SurfaceHolder holder, boolean newSurface) {
            ...
            if (!newSurface) {
                ...
                // message is forthcoming.  Finish the offscreenSurface setup now.
                //
                // We could also just call this unconditionally, and perhaps do an unnecessary
                // bit of reallocating if a offscreenSurface-changed message arrives.
         ...
         * <p>
         * We always receive surfaceChanged() after sendSurfaceCreated(), but surfaceAvailable()
        ...
        private void surfaceDestroyed() {
            ...
            // before the offscreenSurface is destroyed.  In theory it could be called though.
    ...
    private static class RenderHandler extends Handler {
        ...
        /**
         * Sends the "offscreenSurface available" message.  If the offscreenSurface was newly created (i.e.
         * this is called from sendSurfaceCreated()), set newSurface to true.  If this is
         * being called during Activity startup for a previously-existing offscreenSurface, set
         * newSurface to false.
         * <p>
        ...
        /**
         * Sends the "offscreenSurface changed" message, forwarding what we got from the SurfaceHolder.
         * <p>
         * Call from UI thread.
         */
